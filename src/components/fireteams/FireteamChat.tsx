...
export function FireteamChat({ fireteam }: FireteamChatProps) {
  const { user: authUser } = useUser();
  const firestore = useFirestore();
  const [isSending, setIsSending] = useState(false);

  const messagesCollectionRef = useMemoFirebase(
    () => collection(firestore, `fireteams/${fireteam.id}/messages`),
    [firestore, fireteam.id]
  );
  ...
  async function onSubmit(values: z.infer<typeof chatSchema>) {
    ...
    try {
      // Use non-blocking add to keep UI responsive
      addDocumentNonBlocking(messagesCollectionRef, newMessage);
      form.reset();
    } catch (error) {
      console.error('Failed to send message:', error);
    } finally {
        // Even though it's non-blocking, we can reset the sending state quickly
        setIsSending(false);
    }
  }
  ...
  return (
    <Card className="h-full flex flex-col">
      <CardHeader>
        <CardTitle>Fireteam Comms</CardTitle>
      </CardHeader>
